...
def _version() -> str:
    ...
    _buildinfo is generated by FiveAtlas.spec, so a packaged app reports the same
    number as its filename and its Info.plist. Reading FIVEATLAS_VERSION from the
    environment does NOT work once frozen -- it is set while building, not while
    running -- so the packaged app used to fall through to the literal and stamp
    the wrong version into every region file it touched.
    """
    try:
        from _buildinfo import VERSION as baked      # written at build time
        return baked
    except Exception:
        return os.environ.get("FIVEATLAS_VERSION") or "0.4.1"
